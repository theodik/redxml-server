module RedXML
  module Server
    module Transformer
      # Raised every time there is a prolem with mapping, for example name
      # cannot be mapped to id or id cannot be found to be mapped back to name
      class MappingException < StandardError
      end

      # Raised when you try to build wrong/unacceptable key, for example when
      # you have only root key and calls KeyElementBuilder#parent method,
      # root has no parent > exception
      ...
      # Raised when next* method is called on some iterator and
