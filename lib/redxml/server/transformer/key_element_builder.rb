    ...
    module Transformer
      ...
      class KeyElementBuilder
        ...
        def parent!#:KeyElementBuilder
          if @elem_str.empty?
            fail Transformer::NoElementError,
              "There is no parent element for root element."
          end
        ...
        def parent #String
          ...
          last_separator = @elem_str.rindex(SEPARATOR)
          "#{@root_key}#{@elem_str.slice(0, last_separator)}"
        end
        ...
        def last_part_only
          if @elem_str.empty?
            ...
            last_separator = @elem_str.rindex(SEPARATOR)
            "#{@elem_str.slice(last_separator+1, @elem_str.length)}"
        end
        ...
        def next_elem #:String
          if @elem_str.empty?
            fail Transformer::NoElementError,
              "There is no child element of root. " \
              "Cannot call next_elem, while root is always only one."
          end
            last_gt = @elem_str.rindex(">")
            order = @elem_str.slice((last_gt+1)..-1).to_i + 1;
            "#{@root_key}#{@elem_str.slice(0, last_gt+1)}#{order}"
        end
        ...
        def prev_elem
          if @elem_str.empty?
            ...
            if order <= MIN_ORDER
              fail WrongOrderError,
                "Cannot decrease order. Order is already #{MIN_ORDER}."
            end
            order -= 1
            "#{@root_key}#{@elem_str.slice(0, last_gt+1)}#{order}"
        end
        ...
        def elem_id#:String
          if @elem_str.empty?
            ...
            dd_split[dd_split.length-1].split('>')[0]
          end
        end
        ...
        def order #:Integer
          if @elem_str.empty?
            fail WrongOrderError,
              "Cannot return order of the root element."
          end
            gt_split = @elem_str.split('>')
             gt_split[gt_split.length-1].to_i
        end
        ...
        def self.text_order(key_str)
          ...
          if gt_split == nil
            fail WrongOrderError, "No text order to return."
          end
        ...
        def self.last_dd_gt_split(key_str)# :nodoc:
          ...
          return nil if dd_split.length < 1
          dd_split[-1].split('>')
        end
        ...
        def self.text_type?(key_str, type)# :nodoc:
          ...
          gt_split = self.last_dd_gt_split(key_str)
          return false if gt_split == nil
          return false if gt_split.length < 3
          gt_split[-2] == type_char
        end
        ...
        def self.text_type(key_str)
          ...
          case gt_split[-2]
          when "t"
             XML::TextContent::PLAIN
          when "c"
             XML::TextContent::COMMENT
          when "d"
             XML::TextContent::CDATA
          else
             false
          end
        end
        ...
        def self.element?(key_str)
          gt_split = self.last_dd_gt_split(key_str)
          gt_split.length < 3 || gt_split == nil
        end
        ...
        def self.build_from_s(key_builder, key_str)#:KeyElementBuilder
          ...
          if key_split.length < 1
            fail NotEnoughParametersError, "#{key_str} cannot be parsed."
          end
          ...
          if key_split.length > 1
            (1..key_split.length-1).each do |index|
              splitted_split = key_split[index].split(">")
              # Does not support text keys,
              # e.g. 1:2>2>t>3 is translated to 1:2>2
