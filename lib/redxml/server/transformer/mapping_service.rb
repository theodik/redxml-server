    ...
    module Transformer
      ...
      class MappingService
        ...
        def create_elem_mapping(elem_name)
          ...
          elem_id = @db_interface.increment_hash(@key_builder.elem_mapping_key,
                                                 Transformer::KeyElementBuilder::ITERATOR_KEY, 1, true)
          ...
          if result
            ...
            logger.info "result in create_elem_mapping = #{result}, hodnota je =  #{@elem_hash[elem_name]}"
          end
          false
        end
        ...
        def self.map_env(db_interface, env_name)
          env_id = self.map_to_id(db_interface, Transformer::KeyBuilder.environments_key, env_name, "Environment")
          env_id
        end
        ...
        def self.map_coll(db_interface, env_id, coll_name)
          coll_id = self.map_to_id(db_interface, Transformer::KeyBuilder.collections_key(env_id), coll_name, "Collection")
          coll_id
        end
        ...
        def self.map_doc(db_interface, env_id, coll_id, doc_name)
          doc_id = self.map_to_id(db_interface, Transformer::KeyBuilder.documents_key(env_id, coll_id), doc_name, "Document")
          doc_id
        end
        ...
        def self.map_env_coll(db_interface, env_name, coll_name)
          env_id = self.map_to_id(db_interface, Transformer::KeyBuilder.environments_key, env_name, "Environment")
          coll_id = self.map_to_id(db_interface, Transformer::KeyBuilder.collections_key(env_id), coll_name, "Collection")
        ...
        def self.map_env_coll_doc(db_interface, env_name, coll_name, doc_name)
          env_id = self.map_to_id(db_interface, Transformer::KeyBuilder.environments_key, env_name, "Environment")
          coll_id = self.map_to_id(db_interface, Transformer::KeyBuilder.collections_key(env_id), coll_name, "Collection")
          doc_id = self.map_to_id(db_interface, Transformer::KeyBuilder.documents_key(env_id, coll_id), doc_name, "Document")
        ...
        def map_elem_name(elem_name)
          ...
          fail Transformer::MappingException, "Element #{elem_name} does not have mapping to id" unless elem_id
          elem_id
        end
        ...
        def map_attr_name(attr_name)
          ...
          fail Transformer::MappingException, "Attribute #{attr_name} does not have mapping to id" unless attr_id
          attr_id
        end
        ...
        def self.unmap_env(db_interface, env_id)
          env_mapping_key = Transformer::KeyBuilder.environments_key
          env_name = self.unmap_hash(db_interface, env_mapping_key, env_id)
          env_name
        end
        ...
        # environment with such an ID does not exist
        def self.unmap_coll(db_interface, env_id, coll_id)
          coll_mapping_key = Transformer::KeyBuilder.collections_key(env_id)
          coll_name = self.unmap_hash(db_interface, coll_mapping_key, coll_id)
          coll_name
        end
        ...
        # environment (or collection) with such an ID does not exist
        def self.unmap_doc(db_interface, env_id, coll_id, doc_id)
          doc_mapping_key = Transformer::KeyBuilder.documents_key(env_id, coll_id)
          doc_name = self.unmap_hash(db_interface, doc_mapping_key, doc_id)
          doc_name
        end
        ...
        # ==== Raises
        # Transformer::MappingException - If the environment or collection does not exist
        def self.unmap_env_coll(db_interface, env_id, coll_id)
          env_mapping_key = Transformer::KeyBuilder.environments_key
          env_name = self.unmap_hash(db_interface, env_mapping_key, env_id)
          ...
          {ENV_KEY => env_name, COLL_KEY => coll_name}
        end
        ...
        # ==== Raises
        # Transformer::MappingException - If the environment, collection or document does not exist
        def self.unmap_env_coll_doc(db_interface, env_id, coll_id, doc_id)
          env_mapping_key = Transformer::KeyBuilder.environments_key
          env_name = self.unmap_hash(db_interface, env_mapping_key, env_id)
        ...
        def self.map_to_id(db_interface, key, name, description)#:nodoc:
        ...
        def self.unmap_hash(db_interface, hash_key, id)#:nodoc:
