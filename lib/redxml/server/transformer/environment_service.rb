    ...
    module Transformer
      ...
      class EnvironmentService
        ...
        def create_environment(name)
          ...
          fail Transformer::MappingException, "Environment with such a name already exist." unless result
        ...
        def get_environment_id(name)
          ...
          fail Transformer::MappingException, "Environment with such a name doesn't exist." unless env_id
          env_id
        end
        ...
        def rename_environment(old_name, name)
          ...
          fail Transformer::MappingException, "Environment with such a name already exist." if result
          result = @db_interface.hash_value_exist?(@env_key, old_name)
          fail Transformer::MappingException, "Cannot rename, environment #{old_name} doesn't exist." unless result

          #Delete old enevironment
          old_id = get_environment_id(old_name)
          del_res = @db_interface.delete_from_hash(@env_key, [old_name])
          add_res = @db_interface.add_to_hash_ne(@env_key, name, old_id)

          #Note: result may obtain some old return values from redis, we have to lookup at the end of result
          fail Transformer::MappingException, "Cannot delete old environment's name, rename aborted." if del_res != 1 && !del_res
        ...
        def environment_exist?(name)
          @db_interface.hash_value_exist?(@env_key, name)
        end
      end
    end
  end
