        ...
        def end_element_namespace(name, prefix = nil, uri = nil)
          ...
          @path.each do |path|
            ...
            if(info.length < 2)
              ...
              key = RedXML::Server::Transformer::KeyElementBuilder.build_from_s(@mapping_service.key_builder, key.to_s).elem(info[0], info[1])
            end
          end
          ...
          if(key == "")
            ...
            key = RedXML::Server::Transformer::KeyElementBuilder.build_from_s(@mapping_service.key_builder, key.to_s).elem(name, order)
          end
